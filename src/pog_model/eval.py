...
def evaluate_regression(y_true, y_pred_log, name: str):
    ...
    if n == 0:
        return {
            f"{name}_mae_log1p": np.nan,
            f"{name}_n": 0,
        }

    y_true = np.asarray(y_true[mask], dtype=float)
    y_pred_log = np.asarray(y_pred_log[mask], dtype=float)
...
def topk_summary(df: pd.DataFrame, score_col: str, k: int = 50):
    if len(df) == 0:
        return {
            "k": k,
            "avg_expected_pog_prize": np.nan,
            "sum_actual_pog_prize": np.nan,
            "n_win_flag": np.nan,
            "n_bt_place_flag": np.nan,
            "n_graded_win_flag": np.nan,
        }

    top = df.sort_values(score_col, ascending=False).head(k)
    return {
        "k": k,
        "avg_expected_pog_prize": top["expected_pog_prize"].mean() if "expected_pog_prize" in top.columns else np.nan,
